...
Let's have a look at the content of the `survey_df_c`. The variables (i.e. the questions asked in the survey) are shown in the rows of this output. Then the output shows the first few values of the responses:

```{r}
glimpse(survey_df_c)
```

Note that the first few values of the variable `invest` are empty because we included this question only a few days after having launched the survey.
