...
There are only three input files (`Citizen initiative.csv` which comes straight from Google Forms, `Replacement_of_csv_header.txt` and  `header_lookup.txt`) which can be retrieved from the Github page mentionned above.
...
The preprocessing code is available below. The reader can have a look by clicking on the `Code` button on the right. We choose not to explain explicitely the details of data cleaning and tidying, in order to preserve the clarity of this document.
...
Now that we have imported and cleaned the data, we can start doing some exploratory analysis. In this document, we will often refer to Exploratory Data Analysis as the acronym EDA.
...
For each of the two independent variables, we asked a set of questions which, in our opinion, should reflect their corresponding independent variable. In this case, we could say that the independent variables are latent variables that are each measured by a set of several items. We will dig more into the idea of latent variable in the chapter "Confirmatory Factor Analysis".
...
Finally, we asked how much the respondants were already investing in citizens initiatives. This could be regarded as a secondary dependent variable that captures the interest in citizens initiatives.

Note that this feature was added a bit later after having launched the survey online, so the first few observations do not contain information for this question.

This question, along with its corresponding choices and R variable names, is shown in the table below.

```{r}
x <- list(q = names(lut_c_invest_eng),
     v = levels(survey_df_c$invest)[2:6]) %>% as_data_frame()
colnames(x) <- c(header_lookup$google_f_header[28], "variable_name")
x %>% 
  kable() %>%
  kable_styling(bootstrap_options = c("striped","hover"),
                full_width = F, position = "left", font_size = 14)
...
We can break down the distribution of answers related to the independent variables by interest in citizens initiatives.

We should keep in mind that, as shown previously by the distribution of the interest, the proportion of people "ready to fund" and "not interested" is only 9% and 5% of the sample, respectively.
...
In this chapter, we are going to fit a series of multiple binary logistic regression model (which is the most basic model used to predict a categorical response, such as our two dependent variables "interest" and "past investment"). This is useful because this model will allow us to see which predictors (in our case the independent variable questions) are associated with the likelihood of people to be interested in supporting citizens initiatives.

This kind of model can only predict a target variable which has no more than two classes, thus the term binary. However, our dependent variables have more than two classes (for example, there are five classes for the "interest" variable, from "Not interested" up to "Ready to fund"). Therefore, we will fit several models after having split our dependent variable classes into two groups, in several ways. This is called the "one versus rest" approach. There are of course more complex models that can handle more than two classes in the target variable, but we will keep things simple here.

Note that designing a survey with ordinal categorical variables such as questions with choices on a 5-point Likert scale or age or education makes it easier to interpret the results of the models, as we will see next.

### Logistic regression with Interest as the target variable

#### Model 1

In this section, we will fit models with Interest (in supporting citizens initiatives) as the target variable and all of the dependent variables, as well as the control variables, as the explanatory variables.
...
In the present case, all p-values are about 1, so this model is useless. In fact, the model did not converge. This is probably due to the high imbalance in the response variable. Indeed, as already mentionned, the class "Not_interested" represents only 5% of the sample or 7 out of 152 observations.

Therefore, we cannot infer anything regarding factors that are associated with the likelihood of people not to be interested in citizens initiatives as opposed to being somewhat interested ("Could be interested) or definetly interested ("Is ready").
...
This chapter is optionnal for the reader, because we are not confident that our results are correct. We intend to update this part of the document in the future as our knowledge about dimensionality reduction techniques, factor analyis and structural equation modeling improves.

By design, there are questions that are intended to collectively measure one latent variable. For example, there are five questions related to the willigness from the citizens to participate in debates/face-to-face interations. Also, there are six questions related to whether a civic crowdfunding platform could be regarded as an intermediary between the citizens initiatives and the local authorities.

Confirmatory Factor Analysis (CFA) allows us to reduce the dimensionality of several items into their corresponding latent variable. The resulting latent variable is a continuous numeric variable that captures the variability of its likert scale (thus categorical) items.

Let's try it out.

### Preprocessing

Here below are the questions related to the two latent variables (p for participative and g for relationship between citizens initiatives and the government):
...
First we need to describe the model that we want to fit. We will use the variable names `partcip_will` and `civic_crowd_gov` to refer to the willigness from the citizens to participate in debates/face-to-face interations and whether a civic crowdfunding platform could be regarded as an intermediary between the citizens initiatives and the local authorities, respectively.
...
Next, we can fit this model and look at the summary statistics.
...
Modification indices can be requested in order to determine if changes should be made to the model such as adding error covariance terms or removing poor performing items. Here, we sorted the output by `mi` in decreasing order.
...
Instead of qualitatively assess the improvement, we can perform a significance test to compare the two models.

```{r}
anova(fit1, fit2)
```

As evidenced by the p-value way below 0.05, adding the covariance term significantly improved the model. We will then consider the latter model for the rest of this analysis.

We can now save the latent variables and add to our dataset. In the following two tables we show, for the first six observations, the items p1 to p5 and their latent variable, as well as the items g1 to g6 and their own latent variable.
...
Let's see now if we can better assess the relationship between our independent variables (`partcip_will` and `civic_crowd_gov`) and our dependent variable, which is the degree interest of citizens in joining a civic crowdfunding platform.

Our independent variables are now in the form of a continuous numeric value, thus we can use boxplots to explore these relationships.
...
* not being interested in joining a civic crowdfunding platform is associated with negative values of both independent variables
* there does not seem to be a significant difference between the other categories of interest: they are all associated with values of the latent variable centered at or above zero.

Therefore, we will choose to regroup the `interest` variable into two categories:

* no interest (`Not_interested`)
* weak or strong interest (`Could_be_interested_Vol`, `Could_be_interested_Fund`, `Is_ready_Vol`, `Is_ready_Fund`)

This will allow us to fit a binary logistic regression model in the next section.
...
Here again, our dependent variable has more than two classes (three in this case: "Not interested", "Could be interested" and "Ready to join"). Therefore, we will fit several models after having split our dependent variable classes into two groups, in several ways.
...
* Class 0: "Not_interested";
* Class 1: The rest of the possible classes ("Could be interested" and "Ready to join") combined.
...
Therefore, we cannot infer anything regarding factors that are associated with the likelihood of entrepreneurs not to be interested in joining a civic crowdfunding platform as opposed to being somewhat interested ("Could be interested) or definetly interested ("Is ready").
...
Again, the model did not converge. This is probably due to the fact that the number of observations for the entrepreneurs is too small overall to build any logistic regression model.
