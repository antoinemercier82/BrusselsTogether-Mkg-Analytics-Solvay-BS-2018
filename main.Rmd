...
This aim of this document is to provide an analysis of survey data in the context of a the market research done by students at ULB/Sovay Brussels School on citizen initiatives and, in particular, civic crowdfunding. This research originated from a need by BrusselsTogether to better understand the various factors that could be associated with the success of their own civic crowdfunding platform.

The present document provides all of the associated code written in R programming language in order for anyone to be able to check and reproduce the findings presented here.
...
All of the input files, as well as the R code, can be retrieved on our [Github page](https://github.com/antoinemercier82/BrusselsTogether-Mkg-Analytics-Solvay-BS-2018).
...
The preprocessing code (essentially cleaning and tidying of the data) is available below. The reader can have a look by clicking on the `Code` button on the right. We chose not to explicitely detail this process in order to preserve the clarity of this document.
...
First, we can create a bar plot showing the count of respondants, broken down into citizens/entrepreneurs, gender and language:
...
The interest to support citizens initiatives is our dependent variable (this is the outcome that BrusselsTogether would like to better understand) and is measured by the question shown in the table below. Also shown in this table is the related variable name in the R code.
...
Finally, we asked how much the respondants were already investing in citizens initiatives. This could be regarded as a secondary dependent variable capturing the interest in citizens initiatives.
...
Note that the respondants were given the possibility to fill the whole questionnaire in either English, Dutch or French.
...
We can observe from this plot that there is a possible positive relationship between education and the extent to which respondants agreed with the statement "Personally funding a citizen’s initiative is enough to satisfy my desire to engage in this project.", although this should be confirmed with the statistical modeling that we will perform in the next chapter.
...
* Class 1: The rest of the possible classes (`Could_be_interested_Vol`, `Could_be_interested_Fund`, `Is_ready_Vol` and `Is_ready_Fund`) combined.
...
Answers to the question about the age of the respondant range from 1 (`Under_14`) to 9 (`75_plus`).
...
This confirms visually the positive relationship identified in point 1, and it seems to make more sense than for point 5 when interpreting the model 2.
...
In our last model, we are going to change our target variable. Instead of modeling the interest in citizens initiatives explained by all of the other variables, we will be modeling the respondants past investment in citizens initiatives explained by all of the other variables (independent variables and control variables).
...
Confirmatory Factor Analysis (CFA) allows us to reduce the dimensionality of several items into their corresponding latent variable. The resulting latent variable is a continuous numeric variable that captures the variability of its corresponding items.
...
Instead of qualitatively assess this improvement, we can perform a significance test to compare the two models.
...
We can now save the latent variables and add them to our dataset. In the following two tables we show, for the first six observations, the items `p1` to `p5` and their latent variable, as well as the items `g1` to `g6` and their own latent variable.
...
Our independent variables are now reduced to a continuous numeric value, thus we can use boxplots to explore the relationships.
...
The interest to join a civic crowdfunding platform is thus our dependent variable (this is the outcome that BrusselsTogether would like to better understand) and is measured by the question shown in the table below. Also shown in this table is the related variable name in the R code.
...
We can observe from this plot that there is a possible positive relationship between education and the extent to which respondants agreed with the statement "The local government is able to identify the projects that are most wanted by citizens.", although this should be confirmed with the statistical modeling that we will perform in the next chapter.
...
In this section, we will fit a model with Interest (in joining a civic crowdfunding platform) as the target variable and all of the independent variables, as well as the control variables, as the explanatory variables.

In this model, we will split the Interest variable into the two following groups:

* Class 0: `Not_interested`;
* Class 1: The rest of the possible classes (`Could_be_interested` and `Is_Ready`) combined.
...
Again, the model did not converge. This is probably due to the fact that the number of observations for the entrepreneurs is too small overall to build a logistic regression model.

# Conclusions

It is now time to draw conclusions from this study.

First, we should mention that it is not possible to generalize those conclusion to the population of interest, since the respondants were not sampled at random. Furthermore, the sample is biased, as evidenced by the over-representation of respondants with a masters degree for example.
