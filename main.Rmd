...
We have split the data into three tables:

* `survey_df_e` which contains the responses from the social entrepreneurs.
* `survey_df_c` which contains the responses from the citizens.
* `survey_df_j` which contains the responses from both the social entrepreneurs and citizens and from which the likert scale questions have been removed.

### Composition of respondants

We can create a bar plot showing the composition of respondants using `survey_df_j` as follows:
...
### Likert plot for citizens/partivipative questions

Let's have a look at the likert scale questions. The first code chunk below defines the function used to plot the results of the likert scale questions.
...
### Likert plot for citizens/government questions
...
###  Likert plots for citizens/participative questions grouped by interest

Before we investigate the relationship between the answers to the likert scale questions and the interest to volunteer/fund, which is our dependent variable, we should investigate the number of citizens who belong each of the categories for our dependent variable. We can see this distribution in the following plot:
...
###  Likert plots for citizens/government questions grouped by interest
...
###  Likert plots for citizens/participative questions grouped by past investment

Before we investigate the relationship between the answers to the likert scale questions and the past investment, which could be another dependent variable, we should investigate the number of citizens who belong each of the categories for this variable. We can see this distribution in the following plot:
...
As we can see from the previous plot, the distribution is highly imbalanced among the categories, and therefore, in the following plots, the percentages for the categories above 100€ represent only a handful of individuals.
...
###  Likert plots for citizens/government questions grouped by past investment
...
survey_df_c_mat %>% 
  select(interest:neighb, age, edu, loc) %>% 
  head(n = 10) %>% 
  kable() %>% 
  kable_styling(bootstrap_options = c("striped","hover"),
                full_width = T)
...
Note that the following variables were not displayed in this table (because they didn't fit into the webpage): `language`, `grouping` and `gender`).
