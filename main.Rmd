...
We should keep in mind that, as shown previously by the distribution of the interest, the proportion of people "ready to fund" and "not interested" is only 8% and 5% of the sample, respectively.
...
Just like in the section above, while examining the following plots, we should keep in mind that, as shown previously by the distribution of "past investment", the proportions are highly imbalanced, since people investing "100_199", "200_499" and "500_plus" represent only 6%, 1% and 4% of the sample, respectively.
...
In this chapter, we are going to fit a multiple binary logistic regression model (which is the most basic model used to predict a categorical response, such as our two dependent variables "interest" and "past investment"). This is useful because this model will allow us to see which predictors (in our case the independent variable questions) are driving the likelihood of people to be interested in civic crowdfunding platforms.
...
### Logicstic regression with Interest as the target variable

#### Model 1

In this section, we will fit models with Interest (in civic crowdfunding platforms) as the target variable and all of the dependent variables, as well as the control variables, as the explanatory variables.
...
There is a lot of output, but we are going to focus on the Coefficients table in the middle.
...
Now we have three variables for which the p-value of the coefficient is less than 0.05. Let's extract them into the following table:
...
1. Since the effect size (i.e. the estimate of the coefficient) associated with the variable age is above zero, then older people are more likely to be "ready" to fund or volunteer, after controlling for the other variables in the model.
...
This confirms visually the negative relationship identified in point 2.

3. Finally, people agreeing with the statement "I would be more inclined to contribute financially to a project on a civic crowdfunding platform if I had the possibility to express my disagreement about some aspects of the project." are less likely to be "ready" to fund or volunteer, after controlling for the other variables in the model.

Similar to the second point, let's try to confirm this visually:
...
The negative relationship identified in the third point is less clear from this plot.
